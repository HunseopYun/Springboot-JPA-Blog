...
public class DummyControllerTest {
	...
	public User updateUser(@PathVariable int id, @RequestBody User requestUser) { // Json 데이터를 요청 -> Java Object(MessageConverter의 Jackson 라이브러리가 로 변환해서 받아줌)
		...
		//트랙잭션 어노테이션을 걸면 세이브하지 않아도 값이 자동으로 변경된다.
		//더티 체킹
		return user;
	}
	...
	public User detail(@PathVariable int id) {
		// user/4 를 찾으면 내가 데이터베이스에서 못찾으면 user가 null이 될 것 아냐?
		// 그럼 return null이 리턴이 된다. 그럼 프로그램에 문제가 있는 것이다
		// Optional 로 너의 user 객체를 감사서 가져올테니 null인지 아닌지 판단해서 return 하라
...
//	}
		...
		// 스프링부트 = MessageConverter라는 녀석이 응답시에 자동 작동
